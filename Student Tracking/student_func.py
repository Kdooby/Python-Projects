...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com', 'Algebra'))
            conn.commit()
    conn.close()
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_student WHERE col_email = '{}'""".format(var_course))
